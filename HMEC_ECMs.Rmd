...
dtp122L <- dtp122L[,pAdjustedWithinCellLine := p.adjust(wilcoxPValue,method="fdr")]
...
dtp240L <- dtp240L[,pAdjustedWithinCellLine := p.adjust(wilcoxPValue,method="fdr")]
...
dtp <- dtp[,pAdjustedAcrosCellLines := p.adjust(wilcoxPValueAcrossCellLines,method="fdr")]
...
###Signals by ECM Protein and CellLine
...
p <- ggplot(l3, aes(x=factor(ECMp), y=Spot_PA_SpotCellCountNormCOL1MedianNorm, colour=ECMp=="COL1", fill=pAdjustedWithinCellLine<.2))+
  geom_boxplot(outlier.size = .5)+
...
p <- ggplot(l3, aes(x=factor(ECMp), y=Spot_PA_SpotCellCountNormCOL1MedianNorm, fill=pAdjustedAcrosCellLines<.2, colour=CellLine))+
  geom_boxplot(outlier.size = .5)+
    scale_fill_manual(values=c("transparent","cornflowerblue"))+
...
###Proliferation and Lineage Signals

```{r}
#ECMp Proliferation Boxplots
p <- ggplot(l3, aes(x=factor(ECMp), y=Nuclei_PA_Gated_EdUPositiveProportionNormCOL1MedianNorm))+
  geom_boxplot(outlier.size = .5)+
  xlab("Well Plate Barcode")+ylab("Proliferation Proportion")+
  ggtitle("Normalized Proliferation Proportion by ECM Protein")+
    facet_wrap(~CellLine)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.8)),legend.title=element_text(size = rel(.8)))
p <- p + geom_hline(yintercept = 1, colour="blue", alpha=.5)
print(p)
...
p <- ggplot(l3, aes(x=factor(ECMp), y=Cytoplasm_PA_Gated_KRT19PositiveProportionNormCOL1MedianNorm, colour=CellLine))+
  ...
  ggtitle("Normalized KRT19 High Proportion by ECM Protein")+
    facet_wrap(~CellLine)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.8)),legend.title=element_text(size = rel(.8)))
p <- p + geom_hline(yintercept = 1, colour="blue", alpha=.5)
print(p)
...
***
These boxplots show the fully normalized values for spot cell count, EdU positive proportion and KRT19 high positive proportion for each ECM protein and cell line. Each boxplot contains data from 80-120 replicate spots that come from 8 different plates.  


### of the significant signals (Wilcoxon Rank Sum tests)


```{r WilcoxonHeatmapsOfReplicates, fig.height=5}

signalName <- "Nuclei_PA_Gated_EdUPositiveProportionNormCOL1MedianNorm"
dtp <- l3[,.(wilcoxPValue=wilcoxpValuesAcrossCellLines(.SD)),by="ECMp",.SDcols=c("CellLine",signalName)]
dtp <- dtp[,pAdjusted := p.adjust(wilcoxPValue,method="fdr")]
dtf <- l3[l3$ECMp %in% dtp$ECMp[dtp$pAdjusted<0.2],]
...
kable(unique(l3[l3$pAdjustedWithinCellLine<.2,.(CellLine, ECMp, pAdjustedWithinCellLine)]))
```
